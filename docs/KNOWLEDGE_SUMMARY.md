# Knowledge Summary — Paris Part 1: From Lutetia to the Eve of Revolution
...
For Paris Part 1, maintain these style anchors across ALL services:
```
...
```
(Derived from NICHE_STYLE_GUIDE Section 5)
...
**Paris-specific application:** Dissolves for era transitions (Roman → Medieval → Renaissance). Match cuts for architectural evolution (same location, different century). J-cuts for voiceover bridging across time periods.

---

## 6. Retention Curve Benchmarks (from external research)
...
### Target Retention Curve for Paris Part 1
- **0-30s:** Steep initial drop (expected, unavoidable) — mitigate with Double Hook
- **30s-3min:** Stabilization zone — narrative transportation kicks in
- **3-14min:** Flat/gradual decline — maintained by open loops, pattern interrupts, era transitions
- **14-17min:** Slight uptick at conclusion — curiosity about "what happened next" (Part 2 tease)
- **Target average:** 45-55% (above channel benchmark for history content)
...
### Estimation Factors
- Average 17-min script: ~20,000-25,000 narration characters
...
1. **Kling subscription pricing in COSTS_AND_LIMITS.md competitor table** lists Kling at "$6.99-25.99/mo" which reflects lower tiers, not the Ultra $180/mo used in production. No conflict — different tiers.
2. **Flow Veo 3.1 Fast** — 20 credits per output. Still the cheapest video generation option on Ultra (5x cheaper than Quality). Primary iteration and economy B-roll tool.
3. **Content policy blocks on Flow** — terms like "siege", "weapon", "blood" relevant to Paris history may trigger content filters. Mitigation: test in Fast mode (20 cr, cheap), use euphemisms, fall back to Kling/Sora.
4. **Character consistency** — No AI service maintains character consistency across separately generated clips without workarounds. For Paris Part 1 (no recurring characters, narrator-driven), this is not a concern.
