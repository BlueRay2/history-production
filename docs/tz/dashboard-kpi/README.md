...
| 03 | [task-03-append-snapshot-ingest.md](task-03-append-snapshot-ingest.md) | Ingestion runs table, rolling 45-d backfill, append-only writes with `observed_on`, preliminary-flag overwrite logic, first-run 45-d backfill script | `in-review (r 1)` | pending | pending |
