...
| 04 | [task-04-nightly-ingest-job.md](task-04-nightly-ingest-job.md) | Comprehensive nightly job: per-day per-video × all metrics × all dimensions; resilient to partial failures | `ready-to-merge (codex-degraded, gemini-r2-degraded)` | ⚠️ blocked r1+r2 (sandbox bug — `bwrap: loopback`); coordinator self-review attests fixes pin r1 findings | ✅ accepted r1 (REQUEST_CHANGES → 5 fixes pin'd by tests); ⚠️ r2 deferred (`auth_interactive` rc=41 on large prompts; queued retroactive) |
