...
| 04 | [task-04-nightly-ingest-job.md](task-04-nightly-ingest-job.md) | Comprehensive nightly job: per-day per-video × all metrics × all dimensions; resilient to partial failures | `ready-to-merge (gemini-r3-degraded)` | ✅ r3 ACCEPTED (5.5 xhigh, real review with disk-read access). r1 v2 caught 6 findings (HIGH+4MED+LOW), all fixed; r2 partial → fix → r3 ACCEPTED no new findings. | ✅ r2 ACCEPTED post-r1 (3.1-pro). r3 deferred — Google-side `MODEL_CAPACITY_EXHAUSTED` on 3.1-pro AND 2.5-flash; Codex r3 covers the 1-block delta from r2-accepted commit. Queued retroactive. |
