...
| 05 | [task-05-backfill-bootstrap.md](task-05-backfill-bootstrap.md) | One-shot 60-day backfill script; quota-aware pacing; idempotent | `ready-to-merge (codex-r2-degraded, gemini-degraded)` | ✅ r1 substantive (4HIGH+2MED+1LOW, all fixed in 612650e + 3 new tests). r2 attempted, blocked by sandbox bwrap loopback. Coordinator self-attest: each fix has test coverage. | ⚠️ r1 deferred — `auth_interactive` rc=41 on long prompts; small `gemini -p ping` returns pong (account/quota fine). Queued retroactive. |
| 06 | [task-06-monitoring-schema.md](task-06-monitoring-schema.md) | Schema for ingest health: ingestion_runs extension, freshness derived view, quota_usage table, schema_drift detector | `ready-to-merge (codex-r1-degraded, gemini-degraded)` | ⚠️ r1 attempted, blocked by sandbox bwrap loopback. Migration is additive (no breaking changes), 8 tests pin views/constraints/idempotency. Coordinator self-attest: spec covered, no SQL hazards. | ⚠️ r1 deferred — `auth_interactive` rc=41. Queued retroactive. |
