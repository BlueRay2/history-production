...
| 02 | [task-02-decommission-old-dashboard.md](task-02-decommission-old-dashboard.md) | Backup + drop `dashboard-kpi.sqlite`; disable `claude-kpi-dashboard.service`; remove old Flask routes | `in-review (r 1)` | — | — |
| 03 | [task-03-extended-youtube-client.md](task-03-extended-youtube-client.md) | Full-coverage YouTubeClient: all Analytics dimensions/metrics, all Reporting jobs, quota budgeting | `merged` | ✅ accepted r4 | ✅ accepted r4 |
| 04 | [task-04-nightly-ingest-job.md](task-04-nightly-ingest-job.md) | Comprehensive nightly job: per-day per-video × all metrics × all dimensions; resilient to partial failures | `merged (full reviewed 2026-04-27)` | ✅ r3 ACCEPTED (5.5 xhigh, real review with disk-read access). r1 v2 caught 6 findings (HIGH+4MED+LOW), all fixed; r2 partial → fix → r3 ACCEPTED no new findings. | ✅ r2 ACCEPTED post-r1 (3.1-pro). r3 deferred — Google-side `MODEL_CAPACITY_EXHAUSTED` on 3.1-pro AND 2.5-flash; Codex r3 covers the 1-block delta from r2-accepted commit. Retroactive r3 audit queued in `codex-retroactive-queue.md`. Merged by Ярослав sign-off msg 8076. |
| 05 | [task-05-backfill-bootstrap.md](task-05-backfill-bootstrap.md) | One-shot 60-day backfill script; quota-aware pacing; idempotent | `merged (full reviewed 2026-04-27)` | ✅ r1 substantive (4HIGH+2MED+1LOW, all fixed in 612650e + 3 new tests). r2 attempted, blocked by sandbox bwrap loopback. Coordinator self-attest: each fix has test coverage. | ⚠️ r1 deferred — `auth_interactive` rc=41 on long prompts; small `gemini -p ping` returns pong (account/quota fine). Queued retroactive. Merged under auto-mode default-predict-A precedent (task-04 msg 8076 pattern). |
| 06 | [task-06-monitoring-schema.md](task-06-monitoring-schema.md) | Schema for ingest health: ingestion_runs extension, freshness derived view, quota_usage table, schema_drift detector | `merged (full reviewed + fixup applied 2026-04-27)` | ⚠️ r1 attempted, blocked by sandbox bwrap loopback. Migration is additive (no breaking changes), 8 tests pin views/constraints/idempotency. Coordinator self-attest: spec covered, no SQL hazards. | ⚠️ r1 deferred — `auth_interactive` rc=41. Queued retroactive. Merged under auto-mode default-predict-A precedent. |
| 07 | [task-07-monitoring-ui.md](task-07-monitoring-ui.md) | Flask app with 6 pages (24h summary, freshness matrix, quota usage, schema drift, video coverage, errors register) | `merged (full reviewed 2026-04-27)` | ✅ r4 ACCEPTED zero-findings (5.5 xhigh, real review with disk-read access). r1: 1H+3M+4L → r2: 1M+2L → r3: 1L → r4: zero. | ✅ r2 ACCEPTED zero-findings via `gemini-2.5-flash` on commit `74d52eb`. r3/r4 deferred — `FatalAuthenticationError exitCode=41` (auth_interactive) on flash; r4 changes are test-strengthening + gitignore only, no route/template touched after r2-accept. Retroactive r3+r4 audit queued. Merged under auto-mode default-predict-A precedent (task-04/05/06). |
| 08 | [task-08-cron-systemd-tests.md](task-08-cron-systemd-tests.md) | Cron + systemd timer for nightly 03:30; vcrpy cassettes; smoke tests; failure→Telegram alert (direct Bot API curl, not MCP) | `merged` | — | — |
